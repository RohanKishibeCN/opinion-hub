  ...
  return (
    <div className="retro-card p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {rows.length > 0 && <p className="text-xs text-white/60">Last {(rows[rows.length - 1]?.price || 0).toFixed(3)}</p>}
      </div>
      {loading && (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={rows} margin={{ left: -10, right: 16, top: 10 }}>
              ...
              <Tooltip
                ...
                formatter={(val: number, key: string) => (key === "volume" ? [`${val}`, "Volume"] : [val.toFixed(3), "Price"])}
              />
